...
Web Preview 是 CameraSubsystem 的调试辅助链路，不改变核心发布端、订阅端、控制面 IPC、数据面 IPC 的主架构。它不应反向污染核心数据链路，也不应让浏览器直接理解或耦合 CameraSubsystem 内部 IPC 协议。后续的编码保存、目标检测、硬件转换都作为扩展能力预留，不作为第一阶段强依赖。
...
```mermaid
flowchart TB
    subgraph Core["CameraSubsystem 核心链路"]
        Source["CameraSource<br/>USB / UVC / MIPI / 其他后端"]
        Publisher["camera_publisher<br/>核心发布端"]
        ControlIpc["控制面 IPC<br/>Subscribe / Unsubscribe / Ping"]
        DataIpc["数据面 IPC<br/>FrameHeader + FramePayload"]
    end

    subgraph WebExt["extensions/web_preview"]
        Gateway["web_preview_gateway<br/>C++ 独立订阅端"]
        FrameAdapter["FrameAdapter<br/>格式识别 / 元数据补全"]
        Transform["FrameTransformPipeline<br/>可选格式转换 / 缩放 / 硬件加速预留"]
        WebServer["HttpWebServer<br/>静态页面 + WebSocket"]
    end

    subgraph Browser["局域网浏览器"]
        WebUi["Web Preview UI<br/>React / TypeScript"]
        Renderer["FrameRenderer<br/>JPEG / Canvas / WebGL"]
        ControlPanel["ControlPanel<br/>开关流 / 录制 / 检测预留"]
    end

    Source --> Publisher
    Publisher --> ControlIpc
    Publisher --> DataIpc

    Gateway -->|Subscribe / Unsubscribe| ControlIpc
    DataIpc -->|Frame| Gateway

    Gateway --> FrameAdapter
    FrameAdapter --> Transform
    Transform --> WebServer

    WebServer -->|HTTP Static| WebUi
    WebServer -->|WebSocket Binary Frame| Renderer
    ControlPanel -->|WebSocket Control JSON| WebServer
    WebServer --> Gateway
```
...
```mermaid
flowchart LR
    DevPc["开发机<br/>构建前端静态资源"] --> Static["web/dist"]
    Static --> Board["RK3576 开发板"]
    Board --> Gateway["web_preview_gateway<br/>HTTP + WebSocket"]
    PcBrowser["局域网 PC 浏览器"] -->|http://board_ip:8080| Gateway
    PhoneBrowser["局域网其他节点浏览器"] -->|http://board_ip:8080| Gateway
```
...
```mermaid
flowchart LR
    Uvc["USB Camera<br/>JPEG Frame"] --> Publisher["camera_publisher"]
    Publisher --> Gateway["web_preview_gateway"]
    Gateway -->|WebSocket Binary<br/>WebFrameHeader + JPEG Payload| Browser["Browser"]
    Browser --> Decode["Browser JPEG Decode"]
    Decode --> Canvas["Canvas / ImageBitmap 显示"]
```
...
```mermaid
flowchart LR
    Input["CameraFrame<br/>来自数据面 IPC"] --> Normalize["FrameAdapter<br/>统一元数据"]
    Normalize --> Select{"选择处理策略"}

    Select -->|JPEG / MJPEG| JpegPass["JpegPassThrough<br/>原始透传"]
    Select -->|RGB / RGBA| RgbPass["RgbPassThrough<br/>直接送前端"]
    Select -->|NV12 / NV21 / YUYV / UYVY| Transform["FrameTransformPipeline<br/>转换或前端 Shader 渲染"]
    Select -->|未知格式| Unsupported["UnsupportedFrame<br/>状态上报，不崩溃"]

    JpegPass --> Packet["WebFramePacket"]
    RgbPass --> Packet
    Transform --> Packet
    Unsupported --> Packet

    Packet --> WebSocket["WebSocket Binary"]
```
...
```mermaid
flowchart TB
    Main["WebPreviewGatewayMain"]

    Main --> App["WebPreviewGatewayApp<br/>生命周期总控"]
    App --> CameraClient["CameraSubscriberClient<br/>连接 CameraSubsystem"]
    App --> StreamManager["PreviewStreamManager<br/>管理预览流状态"]
    App --> FramePipeline["FramePipeline<br/>格式适配 / 转换 / 丢帧"]
    App --> WebServer["WebServer<br/>HTTP + WebSocket"]
    App --> CommandRouter["CommandRouter<br/>页面命令路由"]

    FramePipeline --> FormatAdapter["FrameFormatAdapter"]
    FramePipeline --> TransformRegistry["FrameTransformerRegistry"]
    TransformRegistry --> JpegTransformer["JpegPassThroughTransformer"]
    TransformRegistry --> CpuTransformer["CpuFrameTransformer"]
    TransformRegistry --> HardwareTransformer["HardwareFrameTransformer<br/>预留 GPU / RGA / NPU 相关扩展入口"]

    WebServer --> ClientManager["WebClientManager<br/>浏览器连接管理"]
    WebServer --> StaticServer["StaticFileServer"]
...
3. `set_record_enabled` 第一阶段只做预留，返回 `not_supported`。
...
flowchart TB
    ...
    App --> Header["Header<br/>CameraSubsystem Web Preview / Gateway 状态"]
    App --> Main["Main Layout"]
    App --> RightPanel["Right Panel<br/>流列表 / 参数 / 预留功能"]

    Main --> Grid["Adaptive Stream Grid"]
    Grid --> TileA["StreamTile<br/>usb_camera_0"]
    Grid --> TileB["StreamTile<br/>future_mipi_0"]
    Grid --> TileC["StreamTile<br/>..."]

    TileA --> Canvas["FrameCanvas"]
    TileA --> Overlay["Overlay<br/>名称 / FPS / 格式 / 状态"]
    TileA --> Actions["Actions<br/>Start / Stop / Record / Detect / Snapshot"]
```
...
```mermaid
flowchart TB
    Publisher["camera_publisher"] --> Gateway["web_preview_gateway<br/>预览订阅端"]
    Publisher --> AiSub["ai_subscriber<br/>外部 AI 订阅端"]

    AiSub -->|DetectionResult<br/>frame_id / boxes / labels| Gateway
    Gateway -->|Frame + Overlay Metadata| Browser["Browser"]
```
...
| Record | 第一版返回 `not_supported`，不实现编码保存。 |
...
| Gateway 内编码保存 | 第一阶段 Record 返回 `not_supported` |
...
- 已确认：Record 按钮第一版只做 UI 预留和 `not_supported` 返回，不在 Gateway 内实现编码保存。
