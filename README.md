...
2. DMA-BUF 零拷贝主链路已完成 RK3576 `/dev/video45` Phase 2 冒烟验证，但仍需补充长稳、慢消费者、多订阅者、异常断连和多平面/RKISP 验证，架构设计见 [docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md](docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md)。
...
4. **MIPI/RKISP 多平面验证**：接入 MPLANE 节点，验证 per-plane fd / offset / stride，并为 RGA/NPU/编码器 import 做准备。
