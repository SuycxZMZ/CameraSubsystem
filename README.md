...
| DMA-BUF 零拷贝主链路 | Phase 1 基础接入 | 已新增 `FrameDescriptor` / `FrameLease` 与 V4L2 `VIDIOC_EXPBUF` 尝试路径；默认仍保留 MMAP + copy fallback，待 RK3576 板端能力验证，设计见 [docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md](docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md) |
...
| [docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md](docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md) | DMA-BUF 零拷贝架构 | 查看 RK3576 / Linux DMA-BUF 主链路演进设计 |
...
2. DMA-BUF 零拷贝主链路已接入 Phase 1 基础代码，但仍需在 RK3576 上验证 `VIDIOC_EXPBUF`、lease 回收和板端 cache 行为；跨进程 `DataPlaneV2` / `SCM_RIGHTS` 尚未实现，架构设计见 [docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md](docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md)。
