...
| H.264 录制编码 | 架构设计中 | 规划独立 `camera_codec_server` 订阅原始流并使用 Rockchip MPP 编码；Web 录制按钮只做控制入口，预览仍显示原始流 |
...
H.264 录制规划为独立 `camera_codec_server` 后台服务：它作为 CameraSubsystem 订阅端接收原始视频流，在板端使用 Rockchip MPP 编码为 H.264 并保存到本地文件。Web 页面实时画面仍由 Web Preview 订阅原始流显示；录制按钮只通过 `web_preview_gateway` 触发后台开始/停止录制并显示状态。
