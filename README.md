...
| H.264 录制编码 | 已打通首版链路 | 独立 `camera_codec_server` 订阅原始流并使用 Rockchip MPP 编码；Web Record 按钮通过 Gateway 触发后台录制，预览仍显示原始流 |
| 板端运行验证 | 阶段完成 | 已在 RK3576 Debian 12 上完成 publisher/subscriber copy 与 DataPlaneV2 smoke test |

---

## 3. 工程结构

![CameraSubsystem 工程结构](docs/images/project-structure.svg)

[打开完整工程结构图](docs/images/project-structure.html)
...
![CameraSubsystem 系统架构](docs/images/system-architecture.svg)

[打开完整系统架构图](docs/images/system-architecture.html)

### 5.2 数据路径

当前数据路径是可运行示例路径，不是最终生产级零拷贝路径：

![CameraSubsystem 数据路径](docs/images/data-path.svg)

[打开完整数据路径图](docs/images/data-path.html)
...
### 5.4 Camera Codec Server 录制架构

H.264 录制已通过独立 `camera_codec_server` 后台服务打通首版链路：它作为 CameraSubsystem 订阅端接收原始视频流，在板端使用 Rockchip MPP 编码为 H.264 并保存到本地文件。Web 页面实时画面仍由 Web Preview 订阅原始流显示；录制按钮通过 `web_preview_gateway` 触发后台开始/停止录制并显示状态。

第一阶段以 USB 摄像头为输入打通链路，即 JPEG/MJPEG 解码后送入 H.264 encoder；MIPI/RKISP 后续按 NV12 / DMA-BUF / MPP import 的低拷贝路径扩展。

详细设计见：

- [docs/CODEC_SERVER_ARCHITECTURE.md](docs/CODEC_SERVER_ARCHITECTURE.md)
...
2. DMA-BUF 数据面已完成 RK3576 `/dev/video45` Phase 2 冒烟验证，但仍需补充更长时间长稳、异常断连和真实 MIPI/RKISP 出帧验证，阶段性记录见 [docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md](docs/DMA_BUF_ZERO_COPY_ARCHITECTURE.md)。
...
4. **Camera Codec Server 录制架构落地**：先以 USB 摄像头打通 Web 录制控制、JPEG/MJPEG 解码、Rockchip MPP H.264 编码和 `.h264` 文件保存，架构设计见 [docs/CODEC_SERVER_ARCHITECTURE.md](docs/CODEC_SERVER_ARCHITECTURE.md)。
5. **MIPI/RKISP 多平面验证**：接入 MPLANE 节点，验证 per-plane fd / offset / stride，并为 RGA/MPP 低拷贝编码路径做准备。
