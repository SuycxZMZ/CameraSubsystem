...
| H.264 录制编码 | 已打通 Web 控制闭环 | 独立 `camera_codec_server` 订阅原始流并使用 Rockchip MPP 编码；状态回传包含录制时长、文件统计、有效编码 profile 和错误信息 |
...
5. **Camera Codec Server 容器化准备**：评估裸 H.264 到 MP4/MKV 的封装边界，优先明确 MP4 时间基、SPS/PPS 写入和异常停止后的可恢复性。
