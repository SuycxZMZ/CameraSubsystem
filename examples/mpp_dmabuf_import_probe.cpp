/**
 * @file mpp_dmabuf_import_probe.cpp
 * @brief Probe whether Rockchip MPP buffer layer can import V4L2 exported DMA-BUF fds
 *
 * Usage:
 *   ./mpp_dmabuf_import_probe [device_path] [width] [height] [fourcc]
 *
 * Example:
...
void Cleanup(int fd, std::vector<BufferResource>& buffers)
{
    for (auto& buffer : buffers)
    {
        ...
        {
            if (plane.mpp_buffer != nullptr)
            {
                (void)mpp_buffer_put(plane.mpp_buffer);
                plane.mpp_buffer = nullptr;
            }
...
int main(int argc, char* argv[])
{
    ...
    std::printf("mpp_import_probe_device=%s\n", device_path.c_str());
    std::printf("driver=%s\n", cap.driver);
    std::printf("card=%s\n", cap.card);
    std::printf("bus=%s\n", cap.bus_info);
    ...
    {
        std::printf("mpp_dmabuf_import_probe_result=FAIL\n");
        close(fd);
        return 1;
    }
    ...
    uint32_t mpp_import_ok = 0;
    uint32_t mpp_import_fail = 0;
    bool ok = true;
    ...
    for (uint32_t i = 0; i < req.count; ++i)
    {
        ...
        for (uint32_t p = 0; p < plane_count; ++p)
        {
            ...
            info.size = plane_resource.length;
            info.fd = plane_resource.dmabuf_fd;
            info.index = static_cast<int>(i);

            const MPP_RET ret = mpp_buffer_import_with_tag(nullptr,
                                                           &info,
                                                           &plane_resource.mpp_buffer,
                                                           "mpp_dmabuf_import_probe",
                                                           "main");
            if (ret != MPP_OK || plane_resource.mpp_buffer == nullptr)
            {
                ++mpp_import_fail;
                std::fprintf(stderr,
                             "MPP mpp_buffer_import failed: index=%u plane=%u fd=%d size=%u ret=%d\n",
            ...
            std::printf("buffer[%u].plane[%u].mpp_fd=%d\n",
                        i, p, mpp_buffer_get_fd(plane_resource.mpp_buffer));
            std::printf("buffer[%u].plane[%u].mpp_size=%zu\n",
                        i, p, mpp_buffer_get_size(plane_resource.mpp_buffer));
        }
    ...
    std::printf("mpp_import_ok=%u\n", mpp_import_ok);
    std::printf("mpp_import_fail=%u\n", mpp_import_fail);
    std::printf("mpp_dmabuf_import_probe_result=%s\n", ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
