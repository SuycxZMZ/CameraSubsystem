/**
 * @file mplane_dmabuf_probe.cpp
 * @brief Probe V4L2 multiplanar DMA-BUF export capability
 *
 * Usage:
 *   ./mplane_dmabuf_probe [device_path] [width] [height] [fourcc]
 *
 * Example:
...
void Cleanup(int fd, std::vector<BufferMapping>& buffers)
{
...
int main(int argc, char* argv[])
{
    ...
    std::printf("mplane_probe_device=%s\n", device_path.c_str());
    std::printf("driver=%s\n", cap.driver);
    std::printf("card=%s\n", cap.card);
    std::printf("bus=%s\n", cap.bus_info);
    ...
    {
        std::printf("mplane_dmabuf_probe_result=FAIL\n");
        close(fd);
        return 1;
    }
    ...
    std::vector<BufferMapping> buffers(req.count);
    ...
    std::printf("mplane_dmabuf_probe_result=%s\n", ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
