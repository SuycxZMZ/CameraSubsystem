...
1. **DataPlaneV2 异常验证**
   ...
   - 下一步补 release socket 主动断开、publisher 退出清理和 fd 泄漏长稳检查
...
| P2 | DataPlaneV2 异常验证 | 进行中：本机单测已覆盖无效 descriptor fd 清理、无效 release 计数、部分 release 超时回收、重复/未知 release 统计；RK3576 真实进程 subscriber 崩溃 failover 已通过，release socket 主动断开和 fd 泄漏长稳仍待验证 |
...
- [ ] 在 RK3576 上继续验证 DataPlaneV2 真实异常：release socket 主动断开、fd 泄漏长稳和 publisher 退出清理
