...
3. **热插拔能力发现与设备重枚举策略**
   ...
   - M2 受控重绑定已完成最小实现：自动切换只允许在 start callback 的 `Initialize()` / `Start()` 失败路径发生，必须已有可信 `stable_physical_id`、唯一 capture 候选且无 pending lease；正常 `Streaming` 状态不做热切换。M2a 扫描工具与候选匹配单测已接入，M2b 仍待制造节点变化场景做板端实证。
