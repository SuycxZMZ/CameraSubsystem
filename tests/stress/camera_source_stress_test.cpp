 ...
 *
 * 测试流程:
 * 1. 初始化日志系统并解析命令行参数（时长、设备、输出目录）。
 * 2. 创建并启动 FrameBroker，注册多个模拟订阅者。
 * 3. 初始化 CameraSource（V4L2），将采集到的帧发布到 FrameBroker。
 * 4. 每秒统计帧率/分发/队列指标，并从最新帧快照保存图片。
 * 5. 达到测试时长或收到退出信号后，依次停止采集、分发并输出汇总。
 *
 * 结果判定:
 * 1. 程序可在目标时长内正常退出，不出现卡死或无法响应信号。
 * 2. 每秒日志可持续输出 FPS 与分发指标，且数值变化符合预期。
 * 3. 输出目录可看到循环覆盖的样例图片（槽位 0~9）。
 * 
 * 注意:
