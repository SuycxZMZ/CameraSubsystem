...
| P2 | DataPlaneV2 长稳与异常验证 | ✅ 已完成本机异常单测、RK3576 慢消费者/多订阅者验证、subscriber 崩溃 failover、release socket 主动断开、fd 泄漏长稳和 publisher 退出清理验证 |
...
| `rk3576-dataplane-v2-lifecycle-smoke.sh`，双 subscriber 运行 60 秒后先停 publisher | 自动判定 PASS；publisher `v2_sent=2495`、`release_pending=0`、`active_leases=0`、`release_timeout=0`、`lease_exhausted=0`；publisher fd `17 -> 17`，两个 subscriber fd 均 `6 -> 6`；publisher 主动退出后 `remaining_publishers=0`、`remaining_subscribers=0`、`sockets_left=0` |
