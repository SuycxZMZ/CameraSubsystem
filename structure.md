...
1.1 核心目标
    本系统旨在构建一个高性能、低延迟、可扩展的 Camera 数据流基座,作为 AI 推理、
    视频编码、预览显示等上层应用的统一数据来源。
...
2.2 核心组件职责
    ...
    │   组件名称      │                     职责描述                        │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ CameraSource    │ 负责 Camera 设备的初始化、参数配置、流控及原始帧    │
    │                 │ 数据的捕获                                         │
    │                 │                                                     │
    │                 │ 关键特性:                                          │
    │                 │   - 多实例支持 (路数上限由平台能力决定,常见 12/16+ 路)│
    │                 │   - V4L2 完整封装                                  │
    │                 │   - Buffer 预分配与复用                            │
    │                 │   - 设备热插拔支持                                 │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ FrameBroker     │ 系统中枢,管理订阅关系,将帧数据分发给所有订阅者     │
    │                 │                                                     │
    │                 │ 关键特性:                                          │
    │                 │   - 线程池调度 (动态扩展)                          │
    │                 │   - 弱引用管理 (防止内存泄漏)                      │
    ...
    │                 │   - 订阅过滤 (按 Camera ID)                        │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ FrameHandle     │ 帧数据的元数据描述符,包含内存句柄而非数据指针     │
    │                 │                                                     │
    │                 │ 关键特性:                                          │
    │                 │   - C 风格 POD 结构                                 │
    │                 │   - 支持 Stride 和多平面格式                       │
    ...
    │                 │   - 跨语言兼容                                     │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ PlatformLayer   │ 封装操作系统相关接口 (Epoll, Thread, Log)          │
    │                 │                                                     │
    │                 │ 关键特性:                                          │
    │                 │   - 跨平台适配 (Linux/Android)                     │
    │                 │   - 统一错误码                                     │
...
2.4 发布端/订阅端进程模型 (当前落地)
    ...
    当前实现:
        - CameraSessionManager: 核心发布端注册、订阅计数、按路启停
        - 控制面 IPC: CameraControlServer / CameraControlClient
        - 数据面 IPC: 示例协议与双进程帧传输链路
        - 默认设备宏: CAMERA_SUBSYSTEM_DEFAULT_CAMERA (/dev/video0)
...
4.1 Camera Source Layer
    ...
    public:
        ...
        // 设置帧回调 (通常指向 Broker 的 Publish 接口)
    ...
    private:
        ...
        // 成员变量
        uint32_t            camera_id_;
        int                 device_fd_;           // V4L2 device fd
        std::string         device_path_;         // 设备路径 (/dev/videoX)
...
4.2 Frame Broker Layer
    ...
    public:
        ...
        struct Statistics
        {
            uint64_t total_frames_published_;
            uint64_t total_frames_dispatched_;
            uint64_t total_frames_dropped_;
            uint64_t current_queue_size_;
            uint64_t peak_queue_size_;
        };
    ...
    private:
        ...
        struct DispatchTask
        {
            FrameHandle            frame_;
            uint32_t               camera_id_;
            std::shared_ptr<void>  buffer_guard_;  // Buffer 引用计数
            uint8_t                priority_;
        };

        // Worker 线程执行逻辑
        void WorkerThreadLoop();
        void DispatchFrame(const FrameHandle& frame);
        ...
        // 订阅表: Key 为 camera_id,Value 为订阅者弱引用列表
        // 使用 weak_ptr 防止 Broker 阻止 Subscriber 析构
        SubscriptionMap           subscription_map_;
        mutable std::shared_mutex subscription_map_mutex_;  // 读写锁

        // 任务队列 (优先级队列)
        struct TaskComparator
        {
            bool operator()(const DispatchTask& a, const DispatchTask& b) const
            {
                return a.priority_ < b.priority_;  // 优先级高的先出队
            }
        };
        std::priority_queue<
            DispatchTask,
...
5.5 能力探测与建议上限落地流程 (工程可执行)
    ...
    caps = ProbeCapabilitiesOrFallback()
    profile = StreamProfile(1920, 1080, 30, kNV12)
    budget = ResourceBudget(1024MB, 60% CPU, 70% NPU, 20% thermal, 20% margin)
    limits = SuggestLimits(caps, profile, budget)
...
8.2 命名规则
    ...
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 类名            │ 大驼峰 (PascalCase)                                  │
    │                 │ 示例: class CameraSource, class FrameBroker         │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 结构体          │ 大驼峰 (PascalCase)                                  │
    │                 │ 示例: struct FrameHandle, struct CameraConfig       │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 函数/方法       │ 大驼峰 (PascalCase)                                  │
    │                 │ 示例: void StartStreaming(), bool Publish()         │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 成员变量        │ 小写 + 下划线后缀                                    │
    │                 │ 示例: int device_fd_, bool is_running_              │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 局部变量        │ 小写 + 下划线                                        │
    │                 │ 示例: int frame_count, void* buffer_ptr             │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 全局变量        │ g_ + 前缀 + 下划线后缀                               │
    │                 │ 示例: int g_system_instance_count_                  │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 常量/宏         │ 全大写 + 下划线                                      │
    │                 │ 示例: const int kMaxBufferCount = 4                 │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 枚举类          │ 大驼峰 (PascalCase)                                  │
    │ 枚举值          │ 小驼峰 + k 前缀                                      │
    │                 │ 示例: enum class PixelFormat { kNV12, kRGB }       │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 命名空间        │ 全小写 + 下划线                                      │
    │                 │ 示例: namespace camera_subsystem                    │
    ├─────────────────┼─────────────────────────────────────────────────────┤
    │ 文件名          │ 全小写 + 下划线                                      │
    │                 │ 示例: camera_source.h, frame_broker.cpp            │
    └─────────────────┴─────────────────────────────────────────────────────┘
