...
- 缩进统一为 **4 个空格**（不使用 Tab 字符）。
- 大括号独立一行（Allman 风格）。
- 控制语句与圆括号之间保留空格：`if (condition)`。
- 单行语句避免过长，建议 100 列以内。
...
- 与平台相关的实现或适配项使用明确后缀：`*_linux`、`*_rk3576`
- 交叉编译相关配置文件命名清晰，例如 `toolchain_rk3576.cmake`
- 禁止在公共 API 中暴露平台特定缩写

### 2.4 资源池与生命周期命名约定

- 资源池统一使用 `*Pool` 后缀，例如 `BufferPool`、`ThreadPool`
- 生命周期相关统计建议使用 `*_count`、`*_in_use` 等清晰字段
